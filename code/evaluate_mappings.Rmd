...
```
```{r Filter by read mapping taxonomy}
...
```{r Saving the image to a .svg file and coverage plots to multiple .svg files}
...
```{r Write tsv files w. Refined report files}
...
```{r Species level classifications using bowtie2 alignments for the genera that were not flagged as false positive}
